...
class TestPlans:
    def test_create_plane(self, qase, create_project):
        ...
        res = qase.plans.create(code=create_project, body=body)
        assert res.status_code == 200
        id_ = res.body.get("result").get("id")

        res_delete = qase.plans.delete(code=create_project, uuid=id_)
        assert res_delete.status_code == 200

        res_delete_case = qase.cases.delete(code=create_project, uuid=uuid_case)
        assert res_delete_case.status_code == 200

    def test_get_all_plans(self, qase, create_project):
        """
        Steps:
        1. Create plane
        2. Get all plans
        ...
        res_get_all = qase.plans.get_all(code=create_project)
    ...
    def test_get_specific_plane(self, qase, create_project):
        ...
        res_get_env = qase.plans.get_specific_plane(code=create_project, uuid=id_)
